...
CREATE TABLE IF NOT EXISTS daily_top4(
 ...
 line REAL,token_id TEXT,market_price REAL,model_prob REAL,edge REAL,signal TEXT,result TEXT,settled_at TEXT,
...
class Store:
    def __init__(self,path):
        ...
        with sqlite3.connect(path) as c:
            ...
            cols={r[1] for r in c.execute("PRAGMA table_info(daily_top4)")}
            if "line" not in cols:
                c.execute("ALTER TABLE daily_top4 ADD COLUMN line REAL")

    def save_top4(self,day,rows):
        with sqlite3.connect(self.path) as c:
            for i,r in enumerate(rows[:4],1):
                c.execute("""INSERT INTO daily_top4(snapshot_at,game_date,rank,game_pk,game,selection_text,side,market_type,line,token_id,market_price,model_prob,edge,signal)
                VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?)
                ON CONFLICT(game_date,rank) DO UPDATE SET snapshot_at=excluded.snapshot_at,game_pk=excluded.game_pk,
                game=excluded.game,selection_text=excluded.selection_text,side=excluded.side,market_type=excluded.market_type,line=excluded.line,
                ...
                (datetime.utcnow().isoformat(),day,i,r.get("game_pk"),r["game"],r["selection_text"],r["side"],r["market_type"],r.get("line"),
